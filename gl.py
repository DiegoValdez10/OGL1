...
class Renderer(object):
    ...
    def render(self):
        ...
        if self.activeShader:
            ...
            glUniformMatrix4fv(glGetUniformLocation(self.activeShader, "viewMatrix"), 1, GL_FALSE,
                               glm.value_ptr(self.getViewMatrix()))
            glUniformMatrix4fv(glGetUniformLocation(self.activeShader, "projectionMatrix"), 1, GL_FALSE,
                               glm.value_ptr(self.projectionMatrix))

            # glUniform1f(glGetUniformLocation(self.activeShader, "time"), self.ela)
            glUniform3fv(glGetUniformLocation(self.activeShader, "directionalLight"), 1,
                         glm.value_ptr(self.directionalLight))

        for obj in self.scene:
            if self.activeShader:
                glUniformMatrix4fv(glGetUniformLocation(self.activeShader, "modelMatrix"), 1, GL_FALSE,
